...
(function ($, undefined) {
	$(function () {
		...
		function initMultiselect() {
			if (multiselect) {
				$("select[name^='notify_email']").multiselect();
				$("select[name^='notify_sms']").multiselect();
			}
		}
		...
		function formatCalendars (str, obj) {
			if (str == '0') {
				return str;
			} else {
				return '<a href="index.php?controller=pjAdminCalendars&action=pjActionIndex&user_id='+obj.id+'" >'+str+'</a>';
			}
		}
		...
		if ($("#grid").length > 0 && datagrid) {
			...
			var $grid = $("#grid").datagrid({
				...
				columns: [{text: myLabel.name, type: "text", sortable: true, editable: true},
				          ...
				          {text: myLabel.calendars, type: "text", sortable: true, editable: false, width: 100, renderer: formatCalendars, align: "center"},
				...
				dataType: "json",
				fields: ['name', 'email', 'created', 'role', 'cnt_calendars', 'status'],
				paginator: {
					actions: [
