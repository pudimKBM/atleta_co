	...
	<form action="<?php echo $_SERVER['PHP_SELF']; ?>?controller=pjAdminBookings&amp;action=pjActionExport" method="post" id="frmExportBookings" class="form pj-form">
		...
		<p>
			...
			<span class="inline_block">
				<select name="calendar_id" id="calendar_id" class="pj-form-field w200">
					<option value="">-- <?php __('lblAllCalendars'); ?> --</option>
					<?php
					foreach ($tpl['calendar_arr'] as $k => $v)
					{
						?><option value="<?php echo $v['id']; ?>"<?php echo isset($_POST['calendar_id']) && $_POST['calendar_id'] == $v['id'] ? ' selected="selected"' : null; ?>><?php echo pjSanitize::html($v['title']); ?></option><?php
					}
					?>
				</select>
			</span>
		...
		<p>
			<label class="title"><?php __('lblEvents'); ?></label>
			<span class="inline_block">
		...
		if(isset($_POST['type']) && $_POST['type'] == 'feed') 
		{
			...
			<div class="tsFeedContainer">
				...
				<span class="inline_block">
					<textarea id="bookings_feed" name="bookings_feed" class="pj-form-field h80" style="width: 726px;"><?php echo PJ_INSTALL_URL; ?>index.php?controller=pjAdminBookings&amp;action=pjActionExportFeed&amp;cid=<?php echo $_POST['calendar_id']; ?>&amp;format=<?php echo $_POST['format']; ?>&amp;type=<?php echo $_POST['period'] == 'next' ? '1' : '2'; ?>&amp;period=<?php echo $_POST['period'] == 'next' ? $_POST['coming_period'] : $_POST['made_period']; ?>&amp;p=<?php echo isset($tpl['password']) ? $tpl['password'] : null;?></textarea>
				</span>
			</div>
			<?php
		} 
