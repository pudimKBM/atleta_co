...
if (isset($tpl['status']))
{
	...
	<form action="<?php echo $_SERVER['PHP_SELF']; ?>?controller=pjAdminUsers&amp;action=pjActionUpdate" method="post" id="frmUpdateUser" class="form pj-form">
		...
		<p>
			...
			<span class="inline_block">
				<select name="notify_email[]" class="pj-form-field" multiple="multiple" size="5">
					...
					{
						?><option value="<?php echo $k;?>"<?php echo !empty($tpl['arr']['notify_email']) ? (in_array($k, explode("::", $tpl['arr']['notify_email'])) ? ' selected="selected"' : null) : null;?>><?php echo pjSanitize::html($v); ?></option><?php
					}
					?>
				</select>
			</span>
		...
		<p>
			<label class="title"><?php __('lblReceiveSms'); ?></label>
			<span class="inline_block">
				<select name="notify_sms[]" class="pj-form-field" multiple="multiple" size="5">
					<?php
					foreach (__('receive_sms', true, false) as $k => $v)
					{
						?><option value="<?php echo $k;?>"<?php echo !empty($tpl['arr']['notify_sms']) ? (in_array($k, explode("::", $tpl['arr']['notify_sms'])) ? ' selected="selected"' : null) : null;?>><?php echo pjSanitize::html($v); ?></option><?php
					}
					?>
				</select>
			</span>
